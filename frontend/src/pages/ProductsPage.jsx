    ...
    <div className="products-page">
      ...
      <div className="shop-header">
        <h1 className="shop-title">
          ...
          {manufacturerParam && !tagParam && <span className="tag-filter-badge"> : {manufacturerParam}</span>}
        </h1>
        ...
        <div className="brand-info-box">
          <div className="brand-info-container">
            ...
            <div className="brand-details">
              <h2 className="brand-name">{brandInfo.name}</h2>
              {brandInfo.description && (
                <div className="brand-description">{brandInfo.description}</div>
              )}
              <div className="brand-contact-grid">
                {brandInfo.address && (
                  <p className="brand-contact-item">
                    <span className="brand-contact-icon">📍</span>
                    <strong>آدرس:</strong> {brandInfo.address}
                  </p>
                )}
                {brandInfo.phone && (
                  <p className="brand-contact-item">
                    <span className="brand-contact-icon">📞</span>
                    <strong>تلفن:</strong> {brandInfo.phone}
                  </p>
                )}
                {brandInfo.email && (
                  <p className="brand-contact-item">
                    <span className="brand-contact-icon">✉️</span>
                    <strong>ایمیل:</strong> {brandInfo.email}
                  </p>
                )}
                {brandInfo.website && (
                  <p className="brand-contact-item">
                    ...
                    <a href={brandInfo.website} target="_blank" rel="noopener noreferrer">{brandInfo.website}</a>
                  </p>
                )}
              </div>
            </div>
          ...
          {sizeList.length > 0 && (
            <div className="filter-group">
              ...
              <div className="filter-buttons">
                ...
                {sizeList.map(sz => (
                  <button key={sz} className={`filter-btn ${selectedSize === sz ? 'active' : ''}`} onClick={() => setSelectedSize(sz)}>
                    {sz}
                  </button>
                ))}
              </div>
          ...
          {clayTypeList.length > 0 && (
            <div className="filter-group">
              ...
              <div className="filter-buttons">
                ...
                {clayTypeList.map(cl => (
                  <button key={cl} className={`filter-btn ${selectedClayType === cl ? 'active' : ''}`} onClick={() => setSelectedClayType(cl)}>
                    {cl}
                  </button>
                ))}
              </div>
          ...
          {glazeList.length > 0 && (
            <div className="filter-group">
              ...
              <div className="filter-buttons">
                ...
                {glazeList.map(gl => (
                  <button key={gl} className={`filter-btn ${selectedGlaze === gl ? 'active' : ''}`} onClick={() => setSelectedGlaze(gl)}>
                    {gl}
                  </button>
                ))}
              </div>
          ...
          {usageList.length > 0 && (
            <div className="filter-group">
              ...
              <div className="filter-buttons">
                ...
                {usageList.map(us => (
                  <button key={us} className={`filter-btn ${selectedUsage === us ? 'active' : ''}`} onClick={() => setSelectedUsage(us)}>
                    {us}
                  </button>
                ))}
              </div>
          ...
          {colorList.length > 0 && (
            <div className="filter-group">
              ...
              <div className="filter-buttons">
                ...
                {colorList.map(col => (
                  <button key={col} className={`filter-btn ${selectedColor === col ? 'active' : ''}`} onClick={() => setSelectedColor(col)}>
                    {col}
                  </button>
                ))}
              </div>
                  ...
                  return (
                    <Link to={`/product/${product.id}`} key={product.id} className="product-card-link">
                      <div className="product-card">
                        ...
                        {/* در کارت محصولات */}
                        {(userRole === 'customer' || userRole === 'partner') && salesMode === 'cart' && (
                          <>
                            {getFinalPrice(product) > 0 ? (
                              <>
                                {product.discount > 0 && (
                                  <p className="old-price">{getDisplayPrice(product).toLocaleString()} تومان</p>
                                )}
                                <p className="product-price">{Math.round(getFinalPrice(product)).toLocaleString()} تومان</p>
                              </>
                            ) : (
                              <p className="product-price inquiry-price">📞 تماس بگیرید</p>
                            )}
                          </>
                        )}
                      </div>
