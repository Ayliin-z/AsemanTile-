...
// اضافه کن به فایل blog.js (بعد از مسیرهای دیگه)

// اضافه کن به فایل blog.js - بعد از بقیه مسیرها

// ========== GET /api/blog/tags ==========
router.get('/tags', async (req, res) => {
  try {
    // دریافت همه تگ‌ها از جدول tags
    const result = await query('SELECT name FROM tags WHERE enabled = 1 ORDER BY name');
    const tags = result.rows.map(row => row.name);
    res.json(tags);
  } catch (error) {
    console.error('GET /api/blog/tags error:', error);
    // اگه جدول tags وجود نداشت، از پست‌ها استخراج کن
    try {
      const postsResult = await query('SELECT tags FROM blog_posts WHERE tags IS NOT NULL');
...
// ========== PUT /api/blog/:id ==========
// PUT /api/blog/:id - باید این مسیر وجود داشته باشه
router.put('/:id', async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { title, slug, excerpt, content, image, meta_title, meta_description, tags, is_published, show_on_homepage } = req.body;
    ...
    if (show_on_homepage !== undefined) { updates.push('show_on_homepage = ?'); params.push(show_on_homepage); }
