...
router.post('/', async (req, res) => {
  try {
    ...
    // بررسی تکراری نبودن کد (فقط اگه کد خالی نباشه)
    const existing = await query('SELECT id FROM products WHERE sku = ?', [productCode]);
    if (existing.rows.length > 0) {
      // اگه کد تکراری بود، یک کد جدید با timestamp بساز
    ...
    `, [
      productCode,  // استفاده از productCode جدید
      data.grade || null,
      data.name,
      data.price || 0,
      data.partnerprice || 0,
