...
router.post('/', async (req, res) => {
  ...
  try {
    let { partner_id, items, shipping_cost, notes, expires_in_hours = 48 } = req.body;
    const userId = req.headers['x-user-id'] ? parseInt(req.headers['x-user-id']) : null;

    // ========== اعتبارسنجی و تبدیل partner_id ==========
    let validPartnerId = parseInt(partner_id);
    if (isNaN(validPartnerId) || validPartnerId <= 0) {
      return res.status(400).json({ success: false, error: 'شناسه همکار نامعتبر است' });
    }

    // ========== اعتبارسنجی items ==========
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ success: false, error: 'لیست محصولات الزامی است' });
    }

    // ========== اعتبارسنجی و تبدیل آیتم‌ها ==========
    const validItems = [];
    let totalAmount = 0;
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let productId = parseInt(item.product_id);
      if (isNaN(productId) || productId <= 0) {
        return res.status(400).json({ 
          success: false, 
          error: `شناسه محصول در ردیف ${i+1} نامعتبر است` 
        });
      }
      ...
      if (isNaN(quantity) || quantity <= 0) {
        return res.status(400).json({ 
          success: false, 
          error: `تعداد محصول در ردیف ${i+1} نامعتبر است` 
        });
      }
      ...
      // نام محصول (اگر نداشت، مقدار پیش‌فرض)
      const productName = item.product_name || 'محصول';
    ...
    // ========== اعتبارسنجی shipping_cost ==========
    let validShippingCost = parseFloat(shipping_cost);
    if (isNaN(validShippingCost)) {
      validShippingCost = 0;
    }
    totalAmount += validShippingCost;

    // ========== اعتبارسنجی expires_in_hours ==========
    let validExpiresHours = parseInt(expires_in_hours);
    if (isNaN(validExpiresHours) || validExpiresHours <= 0) {
      validExpiresHours = 48;
    }
    ...
    const random = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
    const quoteNumber = `QF-${year}${month}${day}-${random}`;

    // ========== محاسبه تاریخ انقضا ==========
    const expiresAt = new Date();
    expiresAt.setHours(expiresAt.getHours() + validExpiresHours);
    ...
    const [quoteResult] = await connection.execute(
      ...
      [quoteNumber, validPartnerId, validShippingCost, totalAmount, notes || null, expiresAt, userId]
    );
    
    const quoteId = quoteResult.insertId;

    // ========== درج آیتم‌ها در جدول quote_items ==========
    for (const item of validItems) {
      await connection.execute(
        `INSERT INTO quote_items 
         (quote_id, product_id, product_name, quantity, price, total, created_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
        [quoteId, item.product_id, item.product_name, item.quantity, item.price, item.total]
      );
    }
    ...
    console.log(`📄 پیش‌فاکتور جدید: ${quoteNumber} - مبلغ: ${totalAmount.toLocaleString()} تومان - همکار: ${validPartnerId}`);
...
router.get('/:id/status-logs', async (req, res) => {
  try {
    ...
    console.error('GET /api/quotes/:id/status-logs error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
